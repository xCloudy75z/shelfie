...
- Validate: `survivorId !== mergedId`; both items exist. Else `{ error }`.
- In a `$transaction` (with a generous timeout like the receipt import, since an item may have many purchases):
  1. Snapshot for undo — read the merged-in item's `{ name, normalized, categoryId }`, the **ids of its purchases**, and the **codes of its barcodes** (BEFORE repointing).
  2. `purchase.updateMany({ where: { itemId: mergedId }, data: { itemId: survivorId } })`.
  3. `barcode.updateMany({ where: { itemId: mergedId }, data: { itemId: survivorId } })`.
  4. `item.delete({ where: { id: mergedId } })`.
- Return `{ ok: true, undo }` where `undo = { mergedItem: {name, normalized, categoryId}, purchaseIds: string[], barcodeCodes: string[] }`.
- Revalidate `/prices`, `/month`, `/log`.

**Undo action** `undoMerge(undo: MergeUndo): Promise<{ ok: true } | { error: string }>`:
- In a `$transaction`: recreate the merged-in item (`item.create` with the snapshot's name/normalized/categoryId — a fresh id, which is invisible to the user); repoint `purchaseIds` back to the new id (`updateMany where id in purchaseIds`); repoint `barcodeCodes` back (`updateMany where code in barcodeCodes`).
- Only repoints rows that still exist (a purchase deleted between merge and undo is simply skipped) — same tolerance as Backup undo. Revalidate the three paths.
...
- On success: a non-blocking toast **"Merged ✓ · Undo"**; the Undo button calls `undoMerge(undo)`. After merge, `router.push('?item=<survivorId>')` (or refresh) so the view lands on the surviving item. After undo, refresh.
...
## 8. Open questions for the break-spec pass
