...
| `progress-board.html` | A self-contained, dependency-free live board (auto-refresh, light/dark, per-phase cards driven by a `PHASES` array). Preserves every break-it finding, append-only. | Stand up **once at project start**; update as work happens. This is what the remote owner reviews in a browser. |
