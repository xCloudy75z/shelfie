...
- **✅ FEATURE-COMPLETE (2026-07-13).** Phase C/D (camera scanning) is shipped and **verified on the owner's iPhone** (installed app): scan→identify on Log, the new-item teach-loop, scan→price-story on Prices, stop-on-first-read. That closed the **A→B→C/D roadmap**. Since then one extra feature shipped & verified: the **Month edit-a-purchase pop-up** — tap a purchase → a floating popup to edit/delete (portaled, keyboard/safe-area aware), with a centered "Discard your changes?" confirm. **129 unit tests.** No open build items. Phase C/D adds `html5-qrcode` (dynamically imported), a `BarcodeScanner` overlay, and "📷 Scan" buttons on Log/Prices; the edit popup lives in `app/components/PurchaseEditModal.tsx` + `EditablePurchases.tsx` + `lib/purchase-edit.ts` (spec/plan dated 2026-07-13). Roadmap/decisions: `polish-phases-decisions.txt`.
